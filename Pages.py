...
def editor_content():
    ...
    def refresh_scenes():
        scene_select.set_options({s['scene_id']: s['scene_name'] for s in CRUD_MANAGER.get_all_scenes_joined()})
    ...
    def on_speaker_change(e):
        ...
        sprite_select.set_options({0: 'No Sprite', **{s['sprite_id']: s['expression'] for s in sprites}})
...
def scenes_page_content():
    ...
    def refresh():
        ...
        loc_select.set_options({l['location_id']: l['name'] for l in CRUD_MANAGER.get_all_locations()})
    def add():
    ...
    with ui.row().classes("w-full gap-8"):
        with ui.card().classes("w-1/3"):
            ...
            loc_select = ui.select({}, label="Locations").classes("w-full")
