...
from typing import Optional, List, Tuple, Dict, Any
from nicegui import ui, run, events
from fastapi.responses import RedirectResponse # Added for compatibility
import re
from pathlib import Path
...
def locations_content():
    BG_DIR = Path('assets/bgs'); BG_DIR.mkdir(parents=True, exist_ok=True)
    ...
    def add(name):
        ...
        if name and uploaded_bytes:
            ...
            (BG_DIR/fname).write_bytes(uploaded_bytes)
            CRUD_MANAGER.create_location(name, str(BG_DIR/fname)); uploaded_bytes=None; refresh()
...
def sprites_content():
    SPRITE_DIR = Path('assets/sprites'); SPRITE_DIR.mkdir(parents=True, exist_ok=True)
    uploaded_bytes, uploaded_ext = None, None
    table, char_select, expr_input = None, None, None

    def refresh(): 
        table.rows = CRUD_MANAGER.get_all_sprites_joined()
        char_select.options = {c['char_id']: c['char_name'] for c in CRUD_MANAGER.get_all_characters()}
    async def handle_upload(e): nonlocal uploaded_bytes, uploaded_ext; uploaded_bytes = await e.file.read(); uploaded_ext = Path(e.file.name).suffix; ui.notify("Staged")
    def add():
        nonlocal uploaded_bytes
        if char_select.value and expr_input.value and uploaded_bytes:
            ...
            (SPRITE_DIR/fname).write_bytes(uploaded_bytes)
            CRUD_MANAGER.create_sprite(char_select.value, expr_input.value, str(SPRITE_DIR/fname)); uploaded_bytes=None; refresh()
...
def choices_content():
    ...
    def refresh():
        ...
        scene_select.options = {s['scene_id']: s['scene_name'] for s in scenes}
        events = CRUD_MANAGER.get_all_events()
        event_select.options = {0: 'No Event'}
        event_select.options.update({e['event_id']: e['name'] for e in events})
        event_select.value = 0
...
def editor_content():
    ...
    def refresh_scenes():
        scene_select.options = {s['scene_id']: s['scene_name'] for s in CRUD_MANAGER.get_all_scenes_joined()}
    ...
    def on_speaker_change(e):
        if not e.value: sprite_select.options = {}; return
        sprites = CRUD_MANAGER.get_sprites_by_char(e.value)
        sprite_select.options = {0: 'No Sprite', **{s['sprite_id']: s['expression'] for s in sprites}}
    ...
    with ui.column().classes("w-full"):
        ...
        with ui.row().classes("w-full gap-8"):
            with ui.card().classes("w-1/3"):
                ...
                speaker_select = ui.select({c['char_id']: c['char_name'] for c in CRUD_MANAGER.get_all_characters()}, label="Speaker", on_change=on_speaker_change).classes("w-full")
...
def scenes_page_content():
    ...
    def refresh():
        ...
        loc_select.options = {l['location_id']: l['name'] for l in CRUD_MANAGER.get_all_locations()}
    def add():
    ...
    with ui.row().classes("w-full gap-8"):
        with ui.card().classes("w-1/3"):
            ...
            loc_select = ui.select({}, label="Default BG")
...
def player_content():
    ...
    # UI Elements (Layers)
    bg_image = ui.image().classes("absolute-top-left w-full h-full object-cover -z-10")
    # Sprite container
    sprite_container = ui.element('div').classes("absolute bottom-0 left-1/2 transform -translate-x-1/2 h-4/5 flex justify-center items-end")
    ...
    def show_choices(group_id):
        ...
        choice_container.clear()
        for c in choices:
            # Create a closure to capture the loop variable 'c'
    ...
    def advance():
        ...
        # Sprite
        if line['sprite_path']:
            sprite_image.source = line['sprite_path']
...
@ui.page('/locations')
def locations_page(): admin_layout(locations_content)
@ui.page('/characters')
def characters_page(): admin_layout(characters_content)
@ui.page('/sprites')
def sprites_page(): admin_layout(sprites_content)
@ui.page('/events')
def events_page(): admin_layout(events_content)
@ui.page('/choices')
def choices_page(): admin_layout(choices_content)
@ui.page('/scenes')
def scenes_page(): admin_layout(scenes_page_content)
@ui.page('/editor')
def editor_page(): admin_layout(editor_content)

@ui.page('/play')
def player_page():
    with ui.column().classes('w-full h-screen relative bg-black overflow-hidden'):
        # FIX: Backward compatible redirect
